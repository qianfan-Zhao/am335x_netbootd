...
void tftp_do(struct tftp_conn *conn)
{
	...
	case TFTP_OP_ACK:
		if (conn->data == conn->block) {
			...
			conn->block++;
			ret = safe_read(conn->fd, res_buf + 4, 512,
					conn->convert, &conn->remain_char);
			if (ret < 0) {
				conn->opcode = TFTP_OP_DO_ERR;
				conn->buf = res_buf;
				conn->buflen =
					sprintf(conn->buf, "read failed: %s",
						strerror(errno));
	...
	case TFTP_OP_DATA:
		...
		if (conn->buflen > 0 &&
		    safe_write(conn->fd, conn->buf, conn->buflen, conn->convert,
			       &conn->remain_char) == -1) {
			conn->opcode = TFTP_OP_DO_ERR;
			conn->buf = res_buf;
			conn->buflen = sprintf(conn->buf, "write failed: %s",
					       strerror(errno));
			conn->buf[conn->buflen - 1] = '\0';
			tftp_do(conn);
			return;
	...
	if (sendto(conn->sock, tosent, res_len, 0, (struct sockaddr *)&addr,
		   sizeof(addr)) <= 0) {
...
int process_tftp_req(int sock, uint32_t srvip, struct tftp_conn *conn, int num)
{
	...
	printf("tftp request %s %s, mode %s\n",
	       (type == TFTP_OP_RRQ) ? "read" : "write", name, mode);
	...
	if (bind(sock_conn, (struct sockaddr *)&srvaddr, sizeof(srvaddr)) ==
	    -1) {
